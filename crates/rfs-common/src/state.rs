...
impl StatePaths {
    ...
    fn blob_cache(&self) -> PathBuf {
        self.cache().join("blobs")
    }
    fn directory_cache(&self) -> PathBuf {
    ...
    fn overlay_data(&self) -> PathBuf {
        self.active().join("overlay/data")
    }
    fn overlay_tmp(&self) -> PathBuf {
...
/// State information exposed to clients without revealing persistence details.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl SessionStore {
    ...
    fn metadata(&self) -> Result<SessionMetadata, StateError> {
        let connection = self
            .connection
            .lock()
        ...
        read_session_metadata(&connection, &self.paths.database())
    }
...
/// Opens retained session state without creating files, migrating the schema,
/// acquiring the daemon lock, or changing lifecycle state.
pub fn open_reader(config: Config) -> Result<Box<dyn SessionStateReader>, StateError> {
    Ok(Box::new(StateReader {
        paths: StatePaths::for_reader(&config)?,
    }))
}

/// Opens daemon-owned writable state and establishes a new active session.
pub fn open_daemon(
    config: Config,
    startup: SessionStartup,
) -> Result<Box<dyn DaemonState>, StateError> {
    let paths = StatePaths::from_config(&config)?;
    Ok(Box::new(SessionStore::create(paths, startup)?))
}
...
fn session_view(paths: &StatePaths, metadata: SessionMetadata) -> SessionView {
    SessionView {
        ...
        control_endpoint: paths.control_socket(),
        cache_path: paths.cache(),
        session_path: paths.active(),
    }
}
...
fn retained_session_result(paths: &StatePaths) -> Result<Option<SessionView>, StateError> {
    ...
    let connection =
        Connection::open_with_flags(paths.database(), OpenFlags::SQLITE_OPEN_READ_ONLY)
            .map_err(|source| db_error(&paths.database(), source))?;
    read_session_metadata(&connection, &paths.database())
        .map(|metadata| Some(session_view(paths, metadata)))
}
